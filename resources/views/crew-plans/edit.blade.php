<x-layouts.app title="Plan d’équipage" :crumb="ucfirst($outing->date->translatedFormat('D j M')).' · '.$outing->title.' · '.$plan->boat->name" :back="route('outings.show', $outing)">
    ...
    <div data-crew-editor="{{ json_encode($editor, JSON_UNESCAPED_UNICODE) }}">
    ...
    @push('modals')
        <div class="no-print fixed inset-0 z-[70] bg-navy-950/50 hidden place-items-center p-4" data-validate-modal>
            <form method="POST" action="{{ route('crew-plans.validation.store', [$outing, $plan]) }}" class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6">
                @csrf
